...
namespace Silkroad.Pk2
{
    public class Blowfish
    {
        ...
        /// <summary>
        /// The function divides the 32-bit entry into four eight-bit quarters and uses
        /// the quarters as input to the S-boxes. Outputs are added to modulo 2 ^ 32 and
        /// XOR to create the final 32-bit output.
        /// </summary>
        /// <param name="x"></param>
        /// <returns><seealso cref="uint"/></returns>
        //[MethodImpl(MethodImplOptions.)]
        private uint Feistel(uint x) => ((_sBoxes[0, x >> 24] + _sBoxes[1, (x >> 16) & 0xff]) ^ _sBoxes[2, (x >> 8) & 0xff]) + _sBoxes[3, x & 0xff];

        /// <summary>
        /// Encrypt the left and right blocks (standart 16 rounds)
        /// </summary>
        /// <param name="leftPar"></param>
        /// <param name="rightPar"></param>
        //[MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Encipher(ref uint leftPar, ref uint rightPar)
        {
            var left = leftPar;
            var right = rightPar;
            ...
            {
                right ^= (Feistel(left) ^ _pArray[i]);
                left ^= (Feistel(right) ^ _pArray[i + 1]);
            }
        ...
        /// </summary>
		//[MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void Decipher(ref uint leftPar, ref uint rightPar)
        {
            var left = leftPar;
            var right = rightPar;
        ...
        /// <summary>
        /// Initalize blowfish keys
        /// </summary>
        /// <param name="keyArray">The key array</param>
        public void Initialize(byte[] keyArray) => Initialize(keyArray, 0, keyArray.Length);
        ...
        //[MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Initialize(byte[] keyArray, int offset, int length)
        {
            uint i, j;
            ...
            {
                temp[3] = keyArray[j];
                temp[2] = keyArray[(j + 1) % length];
                temp[1] = keyArray[(j + 2) % length];
                temp[0] = keyArray[(j + 3) % length];
        ...
        /// <summary>
        /// Encodes a stream of data and returns a new array of the encoded data. Returns null if length is 0.
        /// </summary>
        /// <param name="stream">The stream</param>
        public byte[] Encode(byte[] stream)
            => Encode(stream, 0, stream.Length);
        ...
        public byte[] Encode(byte[] stream, int offset, int length)
        {
            ...
            var workspace = new byte[GetOutputLength(length - offset)];
